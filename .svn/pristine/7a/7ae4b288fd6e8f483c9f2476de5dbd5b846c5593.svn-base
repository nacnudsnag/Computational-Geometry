...
int DELTATHETA = 20; //global used to decide how percise theta adjustments should be 
...
void findPath(int x, int y, int theta)
{
	robot roboto = robots[x][y][theta];
        ...
        for (int i = -5; i <= 5; i += 5)
        {
                for (int j = -5; j <= 5; j+= 5) //change to defined constants
                {
                        ...
                        if (robots[x + i/5][y  + j/5][theta].score > addRobot.score &&
			robots[x + i/5][y + j/5][theta].fits == 1
                        && !(i == 0 && j == 0))
                        {
                                addRobot.theta = roboto.theta;
                                addRobot.prevX = x;
				addRobot.prevY = y;
				addRobot.prevAngle = theta;
	...
	addRobot.a.x = roboto.a.x;
        addRobot.a.y = roboto.a.y;
	for (int i = -DELTATHETA; i <= DELTATHETA; i += DELTATHETA * 2)
	{
		addRobot.theta = roboto.theta + i;
		if (addRobot.theta >= 360)
		{
			addRobot.theta -= 360;
		}
		if (addRobot.theta < 0)
		...
		addRobot.cost = roboto.cost + 4;
		addRobot.score = addRobot.cost + dist(addRobot.a, finish);
		if (robots[x][y][addRobot.theta/10].score > addRobot.score &&
	...
	if (!completed) {completed = 1; cout << "NO PATH" << endl;}
        return;
}
